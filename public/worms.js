(() => {
  ...
  function syncChoiceLinks() {
    for (const a of modeButtons) {
      if (!(a instanceof HTMLAnchorElement) || !a.dataset.mode)
        continue;
      const u = new URL(location.href);
      if (a.dataset.mode === "you-vs-jev")
        u.searchParams.delete("mode");
      else
        u.searchParams.set("mode", a.dataset.mode);
      a.href = `${u.pathname}${u.search}`;
    }
    for (const a of weaponButtons) {
      if (!(a instanceof HTMLAnchorElement) || !a.dataset.weapon)
        continue;
      const u = new URL(location.href);
      if (a.dataset.weapon === "bazooka")
        u.searchParams.delete("weapon");
      else
        u.searchParams.set("weapon", a.dataset.weapon);
      a.href = `${u.pathname}${u.search}`;
    }
  }
  var CELL = 2;
  ...
  var ELEV_MIN = -1.2;
  var ELEV_MAX = 1.45;
  var WEAPON_IDS = ["bazooka", "cluster", "mortar"];
  var WEAPON = {
    bazooka: {
      id: "bazooka",
      label: "BAZOOKA",
  ...
  var SHOT_SOUNDS = Array.from({ length: 46 }, (_, i) => `/sounds/shot-${String(i + 1).padStart(2, "0")}.mp3`);
  var shotPool = SHOT_SOUNDS.map((src) => {
  ...
  function playIncoming() {
    ...
    if (shotPool.length > 1 && i === lastShotIndex)
      i = (i + 1) % shotPool.length;
    lastShotIndex = i;
  ...
  land.width = W;
  land.height = H;
  var landContext = land.getContext("2d");
  if (!landContext)
    throw new Error("terrain canvas failed");
  var landCtx = landContext;
  ...
  var weaponButtons = document.querySelectorAll("[data-weapon]");
  var solid = new Uint8Array(TW * TH);
  var state = {
    mode: "you-vs-jev",
    phase: "turn",
    worms: [],
  ...
  function parseWeapon(raw) {
    ...
    if (s === "bazooka" || s === "cluster" || s === "mortar")
      return s;
  ...
  function setWeapon(id) {
    ...
    for (const btn of weaponButtons)
      btn.classList.toggle("is-active", btn.dataset.weapon === id);
  ...
  function burst(px, py, n, color, power = 3.2) {
    for (let i = 0;i < n; i++) {
  ...
  function flightStep(x, y, vx, vy, f, dt) {
  ...
  function shotTarget(id) {
    const foe = living().find((w) => w.id === id);
    return foe ? { x: foe.x, y: foe.y } : null;
  }
  function shotFromSpec(spec, x, y, vx, vy, kind, targetId, fuse = spec.split) {
  ...
  function splitCluster(s) {
    ...
    for (let k = -2;k <= 2; k++) {
      ...
      bits.push(shotFromSpec(spec, s.x, s.y, Math.cos(a) * spd, Math.sin(a) * spd + 0.55, "bomblet", s.targetId, 0));
    }
    return bits;
  }
  function idx(cx, cy) {
  ...
  function setCell(cx, cy, v) {
    if (inGrid(cx, cy) && cy < TH - 14)
      solid[idx(cx, cy)] = v;
  }
  function carve(px, py, radius) {
    const r = radius / CELL;
    const ccx = px / CELL;
    const ccy = py / CELL;
    const r2 = r * r;
  ...
  function fillDisk(cx, cy, rx, ry, v) {
  ...
  function buildLand() {
    ...
    for (let x = 0;x < TW; x++) {
      ...
      let top = TH;
      if (t < 0.46)
        top = (0.1 + t * 0.82 + 0.035 * Math.sin(x / 9)) * TH;
      if (t > 0.5)
        top = Math.min(top, (0.38 + (t - 0.5) * 0.28 + 0.03 * Math.sin(x / 11)) * TH);
      for (let y = Math.floor(top);y < TH - 14; y++)
        setCell(x, y, 1);
    }
    fillDisk(TW * 0.4, TH * 0.52, 92, 64, 0);
    for (let x = Math.floor(TW * 0.46);x < TW * 0.71; x++) {
      const dip = 6 * Math.sin((x - TW * 0.46) / 14);
      for (let y = Math.floor(TH * 0.45 + dip);y < TH * 0.53 + dip; y++)
        setCell(x, y, 1);
    }
  ...
  function paintLand() {
    ...
    for (let cy = 0;cy < TH - 14; cy++) {
      for (let cx = 0;cx < TW; cx++) {
        if (solid[idx(cx, cy)] !== 1)
          continue;
        const surface = cy === 0 || solid[idx(cx, cy - 1)] !== 1;
        const n = cx * 17 + cy * 31 & 7;
        if (surface)
          landCtx.fillStyle = n > 5 ? "#d7b07a" : "#c4a06a";
        else
          landCtx.fillStyle = n > 5 ? "#7a1830" : n > 2 ? "#9a2240" : "#861c36";
  ...
  function rebuildOrder() {
    ...
    if (!ids.length)
      return;
    if (state.turn >= ids.length)
      state.turn = 0;
    if (!ids.includes(state.order[state.turn] ?? -1))
      state.turn = 0;
  }
  function spawnWorms() {
    const spots = [
      ...
      if (spec.split && peaked && vy > 0.35 && i > 16) {
        ...
        for (let k = -2;k <= 2; k++) {
          ...
          for (let j = 0;j < 110; j++) {
            ...
            bvx = b.vx;
            bvy = b.vy;
            if (bx < 4 || bx > W - 4 || by > WATER || by > 4 && isSolid(bx, by))
              break;
          }
  ...
  function bestShot(from, target, loft, weapon = state.weapon) {
    ...
    for (let e = elevMin;e <= elevMax; e += 0.07) {
      for (let p = 0.42;p <= 1; p += 0.08) {
        ...
        for (const ally of living(from.team)) {
          if (ally.id === from.id)
            continue;
          if (Math.hypot(hit.x - ally.x, hit.y - ally.y) < 44)
            err += 80;
        }
        if (err < best.error)
          best = { elev: e, power: p, error: err, hx: hit.x, hy: hit.y };
      }
    }
    return best;
  }
  function reachableStances(from) {
    ...
    for (const dir of [-1, 1]) {
      ...
      while (walked < MOVE_BUDGET - 6) {
        ...
        if (stood == null || blocked(nx, stood) || stood + R >= WATER - 6)
          break;
        x = nx;
        y = stood;
        walked += 8;
        if (walked % 16 === 0 || walked >= MOVE_BUDGET - 8)
          spots.push({ x, y });
      }
    }
    return spots;
  }
  function planFrom(shooter, target, loft, weapon = state.weapon) {
    const facing0 = target.x >= shooter.x ? 1 : -1;
    let best = {
      destX: shooter.x,
      destY: shooter.y,
      facing: facing0,
    ...
    for (const spot of reachableStances(shooter)) {
      const facing = target.x >= spot.x ? 1 : -1;
      const ghost = { ...shooter, x: spot.x, y: spot.y, facing };
  ...
  function weaponScores(shooter) {
    ...
    return WEAPON_IDS.map((id) => {
      ...
      for (const foe of foes) {
        ...
        if (error_px < best.error_px)
          best = { error_px, target_id: foe.id, target_name: foe.name };
      }
      return { id, ...best };
    });
  }
  ...
  function finishShot() {
    ...
    state.pending = null;
    if (!pending || pending.target_id < 0)
      return;
    const log = {
      team: pending.team,
  ...
  function explode(px, py, shot) {
    ...
    for (const w of living()) {
      ...
      if (d < shot.blast) {
        if (state.pending && w.id === state.pending.target_id)
          state.pending.hit = true;
  ...
  async function askJev() {
    ...
    try {
      ...
      const corr = teamLogs(w.team).length ? ` · corr ${Math.round(bias.x)},${Math.round(bias.y)}` : "";
  ...
  function pickBestWeapon(from, target, loft, preferred) {
    let best = preferred ?? "bazooka";
    let bestErr = Infinity;
    const grouped = living(target.team).some((w) => w.id !== target.id && Math.hypot(w.x - target.x, w.y - target.y) < 70);
    for (const id of WEAPON_IDS) {
      const shot = bestShot(from, target, id === "mortar" ? true : loft, id);
      let err = shot.error;
      if (id === "cluster" && grouped)
        err -= 20;
      if (id === preferred)
        err -= 10;
      if (err < bestErr) {
  ...
  function walk(w, dir, dt) {
    if (state.moveLeft <= 0)
      return;
    if (Math.abs(w.tilt) > 0.25 || Math.abs(w.spin) > 0.08)
      return;
    if (!onGround(w.x, w.y))
      return;
    w.crawling = true;
  ...
  function update(dt) {
    ...
    if (state.phase === "settle") {
      state.boomWait -= dt;
      const settled = state.worms.every((w) => !w.alive || Math.abs(w.vy) < 0.22 && Math.abs(w.vx) < 0.5 && Math.abs(w.spin) < 0.08);
    ...
    if (state.phase === "fly" && state.shots.length) {
      ...
      for (const s of state.shots) {
        ...
        if (s.smoke <= 0 && !boom && !split) {
          ...
          addFx(s.x - s.vx / spd * 8, s.y - s.vy / spd * 8, -s.vx * 0.08 + (Math.random() - 0.5) * 0.3, -s.vy * 0.08 - 0.2, 16 + Math.random() * 10, "smoke", s.kind === "bomblet" ? 4 : 6, "rgba(48,44,56,0.5)");
          if (s.burn > 0) {
        ...
        if (boom)
          explode(boom.x, boom.y, s);
        else
          next.push(s);
      }
      state.shots = next;
      if (!state.shots.length && state.phase === "fly")
        finishShot();
      return;
    }
    const w = current();
    ...
    if (state.phase === "charge") {
      ...
      if (activeIsJev()) {
        if (state.charge >= (state.jevAim?.power ?? 0.7) - 0.02)
          fire(state.charge);
    ...
    if (state.keys.right)
      walk(w, 1, dt);
    if (state.keys.up)
      state.elev = clamp(state.elev + 0.03 * dt, ELEV_MIN, ELEV_MAX);
    if (state.keys.down)
      state.elev = clamp(state.elev - 0.03 * dt, ELEV_MIN, ELEV_MAX);
  ...
  function drawWorm(w) {
    ...
    ctx.arc(5, -22.6, 1.05, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#fff";
    ctx.beginPath();
  ...
  function drawArcPreview(w, elev, power, facing) {
    const spec = WEAPON[state.weapon];
    let x = w.x + facing * 16;
    let y = w.y - 8;
    let vx = Math.cos(elev) * facing * power * spec.maxPower;
    ...
    for (let i = 0;i < 70; i++) {
      ...
      if (x < 4 || x > W - 4 || y > WATER || y > 4 && isSolid(x, y))
        break;
    }
  }
  function drawShot(s) {
  ...
  window.addEventListener("keydown", (event) => {
    unlockAudio();
    if (event.code === "ArrowLeft" || event.code === "ArrowRight" || event.code === "ArrowUp" || event.code === "ArrowDown" || event.code === "KeyA" || event.code === "KeyD" || event.code === "Space" || event.code === "Digit1" || event.code === "Digit2" || event.code === "Digit3") {
    ...
    if (event.code.startsWith("Digit")) {
      ...
      if (id && state.phase === "turn" && !activeIsJev())
        setWeapon(id);
    }
    if (event.code === "Space") {
